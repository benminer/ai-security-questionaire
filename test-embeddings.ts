...
async function getEmbeddings(
  ...
  async function callPredict() {
    const instances: aiplatform.protos.google.protobuf.IValue[] = texts.map((e) =>
    ...
    const embeddings: Embedding[] = (predictions as unknown as Prediction[]).map((p, index) => {
...
export function getQuestionsFromCsvs(files: string[]): { questions: string[], answers: AnswerRow[] } {
  ...
  for (const file of files) {
    ...
    if (records.length) {
      ...
      const validQuestions = records.filter((record: { Question: string, Answer: string }) => !!record.Question && !!record.Answer && record.Question.length > 0 && record.Answer.length > 0);

      questions.push(...validQuestions.map((record: { Question: string }) => record.Question));
      answers.push(...validQuestions.map((record: {Question: string, Answer: string}) => ({question: record.Question, answer: record.Answer, id: Answer.hash(record.Answer)})))
    }
  }
